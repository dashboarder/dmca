...
// CP : Maui Change- What would be the random CA patterns for Maui??
// CP : Fix these arrays later

// Used to store the values of DqsPdEn for frequency buckets 0-3
_MCODE_SEGMENT_PREFIX static uint8_t DqsPdEn[DCS_NUM_CHANNELS][DQ_NUM_BYTES][4];

/*
_MCODE_SEGMENT_PREFIX static const uint32_t CA_PRBS7_PATTERNS[CA_NUM_PATTERNS + 8] = {
...
_MCODE_SEGMENT_PREFIX static uint32_t mdllcode_freq_bin_1[DCS_NUM_CHANNELS];  // Save MDLL code used for Bin 1 CA calibration.
...
_MCODE_SEGMENT_PREFIX static int32_t cacal_per_loopchrnk_right[DCS_NUM_CHANNELS * DCS_NUM_RANKS * CACAL_MAX_SWLOOPS][CA_NUM_VREF_MAX][CA_NUM_BITS];
_MCODE_SEGMENT_PREFIX static int32_t cacal_per_loopchrnk_left[DCS_NUM_CHANNELS * DCS_NUM_RANKS * CACAL_MAX_SWLOOPS][CA_NUM_VREF_MAX][CA_NUM_BITS];
...
_MCODE_SEGMENT_PREFIX static uint32_t cksdll_cscal[CA_NUM_VREF_MAX];
_MCODE_SEGMENT_PREFIX static uint32_t cssdll_cscal[CA_NUM_VREF_MAX];
_MCODE_SEGMENT_PREFIX static uint32_t casdll_cscal[CA_NUM_VREF_MAX];
...
_MCODE_SEGMENT_PREFIX static int32_t rddqcal_per_chrnk_right[DCS_NUM_CHANNELS * DCS_NUM_RANKS][RDDQ_NUM_VREF_MAX][DQ_TOTAL_BITS + DMI_TOTAL_BITS];
_MCODE_SEGMENT_PREFIX static int32_t rddqcal_per_chrnk_left[DCS_NUM_CHANNELS * DCS_NUM_RANKS][RDDQ_NUM_VREF_MAX][DQ_TOTAL_BITS + DMI_TOTAL_BITS];
...
_MCODE_SEGMENT_PREFIX static int32_t cacal_vref_panic[DCS_NUM_CHANNELS * DCS_NUM_RANKS * CACAL_MAX_SWLOOPS][CA_NUM_VREF_MAX] = { { 0 } };
_MCODE_SEGMENT_PREFIX static int32_t rddqcal_vref_panic[DCS_NUM_CHANNELS * DCS_NUM_RANKS][RDDQ_NUM_VREF_MAX] = { { 0 } };
...
static void amp_set_dqs_idle_active(uint32_t ch, uint8_t byte);
static void amp_reset_dqs_idle_active(uint32_t ch, uint8_t byte);
#else
...
#endif
static void amp_set_dqs_weak_pden(uint32_t ch, uint8_t freq_bin);
static void amp_reset_dqs_weak_pden(uint32_t ch, uint8_t freq_bin);
...
static void amp_program_ca_vref(uint32_t ch, uint32_t vref, uint32_t vref_pulse_cnt);
	...
	int bootMode, int numChans, int numRanksPerChn)
{
	...
	// chip_id and chip_rev are by default set to Maui B0 for SEG-DV, since they don't perform run time check
	// iBoot will override with correct values at run time to support A1 and B0
	cfg_params.chip_id = 0x8000;
	...
	// Maui Defaults
	cfg_params.clk_period_bin0 = CLK_PERIOD_1200;
	cfg_params.clk_period_bin1 = CLK_PERIOD_800;
	
	// <rdar://problem/15729353>
	cfg_params.ca_num_vref_f0 = CA_NUM_VREF_F0;
	cfg_params.rddq_num_vref_f0 = RDDQ_NUM_VREF_F0;

	cfg_params.ca_num_vref_f1 = CA_NUM_VREF_F1;
	cfg_params.rddq_num_vref_f1 = RDDQ_NUM_VREF_F1;
	cfg_params.wrdq_num_vref = WRDQ_NUM_VREF;
	
	cfg_params.wrdq_apply_NC_algorithm = false;
	
	for(i=0;i< cfg_params.ca_num_vref_f0;i++) {
		cfg_params.ca_vref_values_f0[i] = 0xC + i;
	}
	
	for(i=0;i< cfg_params.rddq_num_vref_f0;i++) {
		cfg_params.rddq_vref_values_f0[i] = 0xBE + i;
	}

	for(i=0;i< cfg_params.ca_num_vref_f1;i++) {
		// Vref Range is 0x1 (Bit 6 of Vref).
		cfg_params.ca_vref_values_f1[i] = (1 << 6) + 0x5 + i;
	}

#if DCS_REG_VERSION == 1
	for(i=0;i< cfg_params.rddq_num_vref_f1;i++) {
		cfg_params.rddq_vref_values_f1[i] = 0xD1 + i;
	}
#else
	for(i=0;i< cfg_params.rddq_num_vref_f1;i++) {
		cfg_params.rddq_vref_values_f1[i] = 0xD5 + i;
	}
#endif
	...
	cfg_params.tWLO = 18; // max = 20ns
	cfg_params.tWLMRD = 21; // min = 40 tCK
	cfg_params.tWLDQSEN = 11; // max = 20 tCK
	
	// TODO : What should tCKCEH be programmed to. Currently programming it to 2CK or 1 MCU clock.
	// TODO : What should tDSTrain be programmed to.
	// tCKCKEH = 2nCK,tVref,Long=200ns tVref,Short=80ns, tCAENT=10ns. The register fields are in terms of number of MCU clocks(1.66ns)
	cfg_params.tDSTrain = 2;
	cfg_params.tCKCKEH = 2;
	cfg_params.tVREF_LONG = 121;
	cfg_params.tCAENT	= 7;
	
	cfg_params.tADR = 15; // max = 20ns
	cfg_params.tMRD = 9; // max = 14ns
	cfg_params.tDQSCKE = 7; // min = 10ns
	cfg_params.tCKEHCMD = 6;
	
	cfg_params.WrLvlDqsPulseCnt = 0; // This sets the number of pulses to 2.
	
	cfg_params.ca_prbs4_pattern = 0x5E26;
	cfg_params.ca_invert_mask = 0x2A;
...
uint32_t pmgr_src_sel;
	...
	#else
		// Call custom API provided in mcu_helper_fxns.c for PMGR changing mcu_clk to 800Mhz
		pmgr_src_sel = (bucket == 3) ? 8 : 5;
...
_ROMCODE_SEGMENT_PREFIX void calibration_save_restore_ca_wrlvl_regs(uint32_t save_or_restore, uint32_t channels, bool freq_bin)
{
	...
	uint32_t freqchngctl0, freqchngctl1;
	...
	if (save_or_restore == CALIB_SAVE) {
		if(freq_bin) {
			...
			if((bit_pos == 0) && (byte_pos == 0)) {	
				...
				for (ch = 0; ch < channels; ch++) {
					...
					cal_save_value(cfg_params.ca_vref_values_f1[vref_opt_ca[ch]] & CA_WRDQ_VREF_CTRL_BITS, CA_WRDQ_VREF_NUM_BITS, &bit_pos, &byte_pos);
				
					// Save optimal Vref for RDDQ
					cal_save_value(cfg_params.rddq_vref_values_f1[vref_opt_rddq[ch]] & RDDQ_VREF_CTRL_BITS, RDDQ_VREF_NUM_BITS, &bit_pos, &byte_pos);
		...
		// Reset bit & byte positions for the first call 
			...
			for (ch = 0; ch < channels; ch++) {
				...
				if(cfg_params.chip_id == 0x8000 && cfg_params.chip_rev < CHIP_REVISION_B0) {
					// Program CA and WRDQ optimal Vref's
					freqchngctl0 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL0_FREQ1(ch));
					CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL0_FREQ1(ch), (freqchngctl0	& 0xFFFF) | (0xE << 16) | (wrdq_vref_f1 << 24));

					freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch));
					CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch), (freqchngctl1	& 0xFFFF0000) | (0xC) | (ca_vref_f1 << 8));
				} else {
					// Program CA and WRDQ optimal Vref's
					freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch));
					CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch), (freqchngctl1	& 0xFFFF0000) | (0xE) | (wrdq_vref_f1 << 8));

					freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch));
					CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch), (freqchngctl1 & 0xFFFF) | (0xC << 16) | (ca_vref_f1 << 24));
				}
			...
			// hardware calibration.
				...
				for (ch = 0; ch < channels; ch++) {
					...
					if(cfg_params.chip_id == 0x8000 && cfg_params.chip_rev < CHIP_REVISION_B0) {
						// Program CA and WRDQ optimal Vref's
						freqchngctl0 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL0_FREQ0(ch));
						CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL0_FREQ0(ch), (freqchngctl0	& 0xFFFF) | (0xE << 16) | (wrdq_vref_f0 << 24));

						freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch));
						CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch), (freqchngctl1	& 0xFFFF0000) | (0xC) | (ca_vref_f0 << 8));
					} else {
						// Program CA and WRDQ optimal Vref's
						freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch));
						CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch), (freqchngctl1	& 0xFFFF0000) | (0xE) | (wrdq_vref_f0 << 8));

						freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch));
						CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch), (freqchngctl1	& 0xFFFF) | (0xC << 16) | (ca_vref_f0 << 24));
					}
...
_ROMCODE_SEGMENT_PREFIX void calibrate_ca(void)
{
	...
	uint32_t cs_sdll_precal_bin0, ck_sdll_precal_bin0;
	uint32_t vt_scl_factor;
	uint32_t good_vref_not_found;
	uint32_t ca_vref_value;
	uint32_t ca_num_vref = (cfg_params.freq_bin == 0) ? cfg_params.ca_num_vref_f0 : cfg_params.ca_num_vref_f1;
	...
	for (ch = 0; ch < cfg_params.num_channels_runCalib; ch++) {
		...
		for (rnk = 0; rnk < cfg_params.num_ranks; rnk++) {
			for (swloop = 0; swloop < cfg_params.cacalib_sw_loops; swloop++) {
				...
				// Update SOC Drive Strengths to match DRAM's FSP (not for Maui A0/A1)
				if((cfg_params.chip_id != 0x8000) || (cfg_params.chip_id == 0x8000 && cfg_params.chip_rev >= CHIP_REVISION_B0))
					run_soc_upd(ch);
				...
				for (vref = 0; vref < ca_num_vref; vref++) {

					ca_vref_value = (cfg_params.freq_bin == 0) ? cfg_params.ca_vref_values_f0[vref] : cfg_params.ca_vref_values_f1[vref];

					// Program Vref setting
					amp_program_ca_vref(ch, ca_vref_value, 0); //vref_pulse_cnt=0
					...
					cksdll_cscal[vref] = 0; 
					cssdll_cscal[vref] = 0; 
					if(cfg_params.disable_coarse_cs_training == false) {
						// CS Training
				...
				if(cfg_params.freq_bin == 1) {
					...
					cal_pmgr_change_mcu_clk(1);		// switch to 800MHz
				}

				// Set DQS weak pull down enable before CA cal exit. This is a SW workaround for Maui A0.
				// <rdar://problem/16947563>
				if(cfg_params.chip_id == 0x8000 && cfg_params.chip_rev < CHIP_REVISION_B0)
					amp_set_dqs_weak_pden(ch, cfg_params.freq_bin);
				...
				// Reset DQS weak pull down enable after CA cal exit. This is a SW workaround for Maui A0.
				// <rdar://problem/16947563>
				if(cfg_params.chip_id == 0x8000 && cfg_params.chip_rev < CHIP_REVISION_B0)
					amp_reset_dqs_weak_pden(ch, cfg_params.freq_bin);
	...
	for (ch = 0; ch < cfg_params.num_channels_runCalib; ch++) {
		for (rnk = 0; rnk < cfg_params.num_ranks; rnk++) {
			for (swloop = 0; swloop < cfg_params.cacalib_sw_loops; swloop++) {
				...
				ca_vref_value = (cfg_params.freq_bin == 0) ? cfg_params.ca_vref_values_f0[vref_ch] : cfg_params.ca_vref_values_f1[vref_ch];
				amp_program_ca_vref(ch, ca_vref_value, 0); //vref_pulse_cnt=0
...
// RdDQ Sequence
////////////////////////////////////////////////////////////////////////////////////////////////
_ROMCODE_SEGMENT_PREFIX void calibrate_rddq()
{
	uint32_t ch, rnk, vref;
	uint32_t good_vref_not_found;
	uint32_t rddq_num_vref = (cfg_params.freq_bin == 0) ? cfg_params.rddq_num_vref_f0 : cfg_params.rddq_num_vref_f1;
	uint32_t rddq_vref_value;
		...
		for (ch = 0; ch < cfg_params.num_channels_runCalib; ch++) {
			...
			for (rnk = 0; rnk < cfg_params.num_ranks; rnk++) {
				...
				for(vref = 0; vref < rddq_num_vref; vref++) {
				
					rddq_vref_value = (cfg_params.freq_bin == 0) ? cfg_params.rddq_vref_values_f0[vref] : cfg_params.rddq_vref_values_f1[vref];

					// Program RDDQ Vref setting
					amp_program_rddq_vref(ch, rddq_vref_value, 0, cfg_params.freq_bin);
...
_ROMCODE_SEGMENT_PREFIX void amp_setup_ca_cal(uint32_t ch, uint32_t rnk, uint32_t swloop)
{
	...
	if(cfg_params.chip_id == 0x8000 && cfg_params.chip_rev < CHIP_REVISION_B0)
		CSR_WRITE(rAMP_CACALCTRL3(ch),(cfg_params.tDQSCKE << 8) | (cfg_params.tCKEHCMD));
	// Also, Program Fine CS parameters
	else
	...
	CSR_WRITE(rAMP_CACALPAT(ch), (cfg_params.ca_invert_mask<<16)|(cfg_params.ca_prbs4_pattern));
}
...
// CP : Maui Change
_ROMCODE_SEGMENT_PREFIX void amp_program_ca_vref(uint32_t ch, uint32_t vref, uint32_t vref_pulse_cnt)
{
...
_ROMCODE_SEGMENT_PREFIX void amp_program_rddq_vref(uint32_t ch, uint32_t vref, uint32_t vref_pulse_cnt, uint8_t freq_bin)
{
	...
	// 2.Vref pulse count
		...
		if(freq_bin == 0) {
			for (byte = 0; byte < DQ_NUM_BYTES; byte++) {
				rddqcalvref = CSR_READ(rAMP_RDDQCALVREF_f0(byte, ch));
				if(cfg_params.chip_id == 0x8000)
					CSR_WRITE(rAMP_RDDQCALVREF_f0(byte, ch),(rddqcalvref & 0xFFFFFF00) | (vref));
				else
					CSR_WRITE(rAMP_RDDQCALVREF_f0(byte, ch),(rddqcalvref & 0xFFFFFF00) | (1 << 7)| (vref));
			}
		}
		else if(freq_bin == 1) {
			for (byte = 0; byte < DQ_NUM_BYTES; byte++) {
				rddqcalvref = CSR_READ(rAMP_RDDQCALVREF_f1(byte, ch));
				if(cfg_params.chip_id == 0x8000)
					CSR_WRITE(rAMP_RDDQCALVREF_f1(byte, ch),(rddqcalvref & 0xFFFFFF00) | (vref));
				else
					CSR_WRITE(rAMP_RDDQCALVREF_f1(byte, ch),(rddqcalvref & 0xFFFFFF00) | (1 << 7) | (vref));
			}
		}
}
...
// (Re-)Initialize CA deskew registers
...
// (Re-)Initialize CA, CS and CK SDLL registers
...
_ROMCODE_SEGMENT_PREFIX void amp_push_cksdll_out(uint32_t ch, uint32_t cksdll_ovr_val)
{
	...
	if (cksdll_ovr_val > (3 * mdllcode[ch][AMP_CA]) + 1) {
		...
		if(cfg_params.chip_id == 0x8000)
			SPIN_W_TMO_WHILE (CSR_READ(rAMP_CKDESKEW_CTRL(ch)) & RUNDESKEWUPD);
...
_ROMCODE_SEGMENT_PREFIX void amp_push_cssdll_out(uint32_t ch, uint32_t cssdll_ovr_val)
{
	...
	if (cssdll_ovr_val > (3 * mdllcode[ch][AMP_CA]) + 1) {
		...
		//SPIN_W_TMO_WHILE (CSR_READ(rAMP_CSDESKEW_CTRL(ch)) & RUNDESKEWUPD);
...
_ROMCODE_SEGMENT_PREFIX void amp_set_dqs_weak_pden(uint32_t ch, uint8_t freq_bin)
{
	uint8_t byte, freq;
	uint32_t rdcapcfg_freq;
	uint32_t dqspdenalwayson;
	
	for(freq=0; freq < 4; freq++) {
		for (byte = 0; byte < DQ_NUM_BYTES ; byte++) {
			DqsPdEn[ch][byte][freq] = (CSR_READ(rAMP_RDCAPCFG_FREQ(ch, byte, freq)) >> DQS_PDEN_OFFSET) & 0x1; 	
		}
	}

	if(freq_bin == 1) {
		// Zero out all PdEn's except for Bin 3
		for(freq=0; freq < 4; freq++) {
			for (byte = 0; byte < DQ_NUM_BYTES ; byte++) {
				if(freq != 3) {
					rdcapcfg_freq = CSR_READ(rAMP_RDCAPCFG_FREQ(ch, byte, freq));	
					CSR_WRITE(rAMP_RDCAPCFG_FREQ(ch, byte, freq), (rdcapcfg_freq & DQS_PDEN_MASK) | (0 << DQS_PDEN_OFFSET));
				}
			}
		}
	}
	else if(freq_bin == 0) {
		// Zero out all PdEn's except for Bin 1
		for(freq=0; freq < 4; freq++) {
			for (byte = 0; byte < DQ_NUM_BYTES ; byte++) {
				if(freq != 1) {
					rdcapcfg_freq = CSR_READ(rAMP_RDCAPCFG_FREQ(ch, byte, freq));	
	...
	// Set DQS PD EN ALWAYSON.
	for (byte = 0; byte < DQ_NUM_BYTES ; byte++) {
		dqspdenalwayson = CSR_READ(rAMP_DQSPDENALWAYSON(ch, byte));
		CSR_WRITE(rAMP_DQSPDENALWAYSON(ch, byte), (dqspdenalwayson & ~DQSPDEN_ALWAYSON_MASK) | (1 << DQSPDEN_ALWAYSON_OFFSET));
	}
}

_ROMCODE_SEGMENT_PREFIX void amp_reset_dqs_weak_pden(uint32_t ch, uint8_t freq_bin)
{
	uint8_t byte, freq;
	uint32_t rdcapcfg_freq;
	uint32_t dqspdenalwayson;

	// Reset DQS PD EN ALWAYSON.
	for (byte = 0; byte < DQ_NUM_BYTES ; byte++) {
		dqspdenalwayson = CSR_READ(rAMP_DQSPDENALWAYSON(ch, byte));
		CSR_WRITE(rAMP_DQSPDENALWAYSON(ch, byte), (dqspdenalwayson & ~DQSPDEN_ALWAYSON_MASK) | (0 << DQSPDEN_ALWAYSON_OFFSET));
	}
	
	if(freq_bin == 1) {
		// Restore all PdEn's except for Bin 3
		for(freq=0; freq < 4; freq++) {
			for (byte = 0; byte < DQ_NUM_BYTES ; byte++) {
				if(freq != 3) {
					rdcapcfg_freq = CSR_READ(rAMP_RDCAPCFG_FREQ(ch, byte, freq));	
					CSR_WRITE(rAMP_RDCAPCFG_FREQ(ch, byte, freq), (rdcapcfg_freq & DQS_PDEN_MASK) | (DqsPdEn[ch][byte][freq] << DQS_PDEN_OFFSET));
				}
			}
		}
	}
	else if(freq_bin == 0) {
		// Restore all PdEn's except for Bin 1
		for(freq=0; freq < 4; freq++) {
			for (byte = 0; byte < DQ_NUM_BYTES ; byte++) {
				if(freq != 1) {
					rdcapcfg_freq = CSR_READ(rAMP_RDCAPCFG_FREQ(ch, byte, freq));	
...
#if DCS_REG_VERSION == 1
_ROMCODE_SEGMENT_PREFIX void amp_set_dqs_idle_active(uint32_t ch, uint8_t byte)
{
	uint32_t dqfltctrl;
	dqfltctrl = CSR_READ(rAMP_AMPSDQIOCFG_DQFLTCTRL(byte, ch));

	dqfltctrl = (dqfltctrl & DQS_IDLE_ACTIVE_MASK) | (1 << DQS_IDLE_ACTIVE_OFFSET); 	

	CSR_WRITE(rAMP_AMPSDQIOCFG_DQFLTCTRL(byte, ch), dqfltctrl);
}

_ROMCODE_SEGMENT_PREFIX void amp_reset_dqs_idle_active(uint32_t ch, uint8_t byte)
{
	uint32_t dqfltctrl;
	dqfltctrl = CSR_READ(rAMP_AMPSDQIOCFG_DQFLTCTRL(byte, ch));

	dqfltctrl = (dqfltctrl & DQS_IDLE_ACTIVE_MASK) | (0 << DQS_IDLE_ACTIVE_OFFSET); 	
...
_ROMCODE_SEGMENT_PREFIX void amp_set_dqs_idle_active(uint32_t ch, uint32_t freq_bin)
{
	...
	if(freq_bin == 0) {
		CSR_WRITE(rAMPH_CFGH_DQS0_WKPUPD(ch), (amph_cfgh_dqs0_wkpupd & F0_IDLE_ACTIVE_MASK) | (1 << F1_PDPWK_OFFSET) | (1 << F0_IDLE_ACTIVE_OFFSET));
		CSR_WRITE(rAMPH_CFGH_DQS1_WKPUPD(ch), (amph_cfgh_dqs1_wkpupd & F0_IDLE_ACTIVE_MASK) | (1 << F1_PDPWK_OFFSET) | (1 << F0_IDLE_ACTIVE_OFFSET));
	}
	else if(freq_bin == 1) {
		CSR_WRITE(rAMPH_CFGH_DQS0_WKPUPD(ch), (amph_cfgh_dqs0_wkpupd & F1_IDLE_ACTIVE_MASK) | (1 << F1_PDPWK_OFFSET) | (1 << F1_IDLE_ACTIVE_OFFSET));
		CSR_WRITE(rAMPH_CFGH_DQS1_WKPUPD(ch), (amph_cfgh_dqs1_wkpupd & F1_IDLE_ACTIVE_MASK) | (1 << F1_PDPWK_OFFSET) | (1 << F1_IDLE_ACTIVE_OFFSET));
	}
}
...
_ROMCODE_SEGMENT_PREFIX void amp_reset_dqs_idle_active(uint32_t ch, uint32_t freq_bin)
{
	...
	// Also make sure that PDPWK_F1 is set.
	if(freq_bin == 0) {
		CSR_WRITE(rAMPH_CFGH_DQS0_WKPUPD(ch), (amph_cfgh_dqs0_wkpupd & F0_IDLE_ACTIVE_MASK) | (1 << F1_PDPWK_OFFSET) | (0 << F0_IDLE_ACTIVE_OFFSET));
		CSR_WRITE(rAMPH_CFGH_DQS1_WKPUPD(ch), (amph_cfgh_dqs1_wkpupd & F0_IDLE_ACTIVE_MASK) | (1 << F1_PDPWK_OFFSET) | (0 << F0_IDLE_ACTIVE_OFFSET));
	}
	else if(freq_bin == 1) {
		CSR_WRITE(rAMPH_CFGH_DQS0_WKPUPD(ch), (amph_cfgh_dqs0_wkpupd & F1_IDLE_ACTIVE_MASK) | (1 << F1_PDPWK_OFFSET) | (0 << F1_IDLE_ACTIVE_OFFSET));
		CSR_WRITE(rAMPH_CFGH_DQS1_WKPUPD(ch), (amph_cfgh_dqs1_wkpupd & F1_IDLE_ACTIVE_MASK) | (1 << F1_PDPWK_OFFSET) | (0 << F1_IDLE_ACTIVE_OFFSET));
	}
}
#endif
...
_ROMCODE_SEGMENT_PREFIX void amp_setup_rddq_cal(uint32_t ch, uint32_t rnk)
{
	...
	tWL = ((cfg_params.WL)/2) - 4; 

	if(cfg_params.freq_bin == 0) {
		CSR_WRITE(rAMP_RdWrDqCalTiming_f0(ch), (rdwrdqcaltiming_f0 & 0xFFFFFF00) | (tWL << 4) | tRL);
	}
	else if(cfg_params.freq_bin == 1) {
		shim_printf("amp_setup_rddq_cal:: Writing a value of 0x%x into rAMP_RdWrDqCalTiming_f1 \n", (rdwrdqcaltiming_f1 & 0xFFFFFF00) | (tWL << 4) | tRL);
		CSR_WRITE(rAMP_RdWrDqCalTiming_f1(ch), (rdwrdqcaltiming_f1 & 0xFFFFFF00) | (tWL << 4) | tRL);
	}
...
_ROMCODE_SEGMENT_PREFIX void amp_wrlvl_entry(uint32_t ch)
{
	...
	for(byte = 0; byte < DQ_NUM_BYTES; byte++ ) {
		amp_set_dqs_idle_active(ch, byte);
	}
...
_ROMCODE_SEGMENT_PREFIX void amp_wrlvl_exit(uint32_t ch, uint8_t freq_bin)
{
	...
	for(byte = 0; byte < DQ_NUM_BYTES; byte++ ) {
		amp_reset_dqs_idle_active(ch, byte);
	}
}
...
_ROMCODE_SEGMENT_PREFIX void cacal_program_final_values(void)
{
	...
	for (ch = 0; ch < cfg_params.num_channels; ch++) {
		...
		*/
		...
		if (max_CaBitCenterPoint_val >= 0) {
			if (ckcs_diff > 0) {
				if (max_CaBitCenterPoint_val > ckcs_diff) {
					...
					amp_program_ck_sdll(ch, max_CaBitCenterPoint_val + (ckcs_diff - max_CaBitCenterPoint_val));
					
					// Since CK SDLL is moved, also program DQS WRLVL SDLL here 
					for(byte = 0 ; byte < DQ_NUM_BYTES; byte++ ) {
						dqwrlvl_sdll_before_wrlvl[ch][byte] = max_CaBitCenterPoint_val + (ckcs_diff - max_CaBitCenterPoint_val);
						amp_program_dqwrlvl_sdll(ch, byte, dqwrlvl_sdll_before_wrlvl[ch][byte], 0);
					}
			...
			ckcs_diff_opt = cksdll_cscal[vref_opt_ca[loopchrnk0_indx]] - cssdll_cscal[vref_opt_ca[loopchrnk0_indx]];

			if(cfg_params.chip_id == 0x8000 && ckcs_diff > 0) {
			...
			} else if(cfg_params.chip_id != 0x8000 && ckcs_diff_opt > 0) {
				amp_program_ck_sdll(ch, ckcs_diff_opt);

				// Since CK SDLL is moved, also program DQS WRLVL SDLL here
				for(byte = 0 ; byte < DQ_NUM_BYTES; byte++ ) {
					dqwrlvl_sdll_before_wrlvl[ch][byte] = ckcs_diff_opt;
					amp_program_dqwrlvl_sdll(ch, byte, dqwrlvl_sdll_before_wrlvl[ch][byte] , 0);
				}

				amp_program_cs_sdll(ch, 0);
				amp_program_ca_sdll(ch, __abs(max_CaBitCenterPoint_val) + ckcs_diff_opt);
			} else {
		...
		ca_vref_value = (cfg_params.freq_bin == 0) ? cfg_params.ca_vref_values_f0[vref_ch] : cfg_params.ca_vref_values_f1[vref_ch];
		shim_mrcmd_to_ch_rnk(MR_WRITE, ch, 0, MR12, ca_vref_value);
		...
		if(cfg_params.freq_bin == 1) {
			...
			if(cfg_params.chip_id ==0x8000 && cfg_params.chip_rev < CHIP_REVISION_B0)
				CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch), (freqchngctl1  & 0xFFFF0000) | (0xC) | (ca_vref_value << 8));
			else
				CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch), (freqchngctl1  & 0xFFFF) | (0xC << 16) | (ca_vref_value << 24));
		}
		else if(cfg_params.freq_bin == 0) {
			freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch));

			if(cfg_params.chip_id ==0x8000 && cfg_params.chip_rev < CHIP_REVISION_B0)
				CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch), (freqchngctl1  & 0xFFFF0000) | (0xC) | (ca_vref_value << 8));
			else
				CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch), (freqchngctl1  & 0xFFFF) | (0xC << 16) | (ca_vref_value << 24));
		}
	}
}
...
_ROMCODE_SEGMENT_PREFIX void rddqcal_program_final_values(void)
{
	...
	for (ch = 0; ch < cfg_params.num_channels; ch++) {
		...
		for (byte = 0; byte < DQ_NUM_BYTES; byte++) {
			...
			rddq_vref_value = (cfg_params.freq_bin == 0) ? cfg_params.rddq_vref_values_f0[vref_ch] : cfg_params.rddq_vref_values_f1[vref_ch];
...
_ROMCODE_SEGMENT_PREFIX int wrdqcal_set_half_clk_delay(uint32_t byte, uint32_t val)
{
  ...
  // TODO (MAUI_WAIVE) : What Reg? 
  //if (isforbyte(0, byte))
  //  rREG_SET_N(wrdqcal_chan, 0) = val;
  //if (isforbyte(1, byte))
  //  rREG_SET_N(wrdqcal_chan, 1) = val;
...
_ROMCODE_SEGMENT_PREFIX int wrdqcal_program_dq_deskew(uint32_t chan, uint32_t byte, uint32_t val)
{
	...
	if (isforbyte(0, byte)) {
	    for (bit_indx = 0; bit_indx < DQ_NUM_BITS_PER_BYTE; bit_indx++) {
		    ...
		    // Poll for deskew update to be done.
		    SPIN_W_TMO_WHILE (CSR_READ(rAMP_WRDQDESKEW_CTRL(0, chan, bit_indx)) & RUNDESKEWUPD);
	    }
		// Also program DMI deskew here.
		CSR_WRITE(rAMP_WRDMDESKEW_CTRL(0, chan), (val) | RUNDESKEWUPD);

		// Poll for deskew update to be done.
		SPIN_W_TMO_WHILE (CSR_READ(rAMP_WRDMDESKEW_CTRL(0, chan)) & RUNDESKEWUPD);	
    }
	...
	if (isforbyte(1, byte)) {
	    for (bit_indx = 0; bit_indx < DQ_NUM_BITS_PER_BYTE; bit_indx++) {
		    ...
		    // Poll for deskew update to be done.
		    SPIN_W_TMO_WHILE (CSR_READ(rAMP_WRDQDESKEW_CTRL(1, chan, bit_indx)) & RUNDESKEWUPD);
	    }
		// Also program DMI deskew here.
		CSR_WRITE(rAMP_WRDMDESKEW_CTRL(1, chan), (val) | RUNDESKEWUPD);

		// Poll for deskew update to be done.
		SPIN_W_TMO_WHILE (CSR_READ(rAMP_WRDMDESKEW_CTRL(1, chan)) & RUNDESKEWUPD);	
    }
...
{
	// Set DQ/DQS for this search point as OVERRIDE (range check included) for the Channel we are calibrating
...
_ROMCODE_SEGMENT_PREFIX int wrdqcal_set_search_point(uint32_t byte, int32_t point)
{
  ...
  uint32_t min_of_4mdll_maxsdll = MAX_SDLL_VAL;
  uint32_t deskew_val;
  ...
  if (((point < 0) && (((uint32_t)__abs(point)) > (4 * mdllcode_byte) || ((uint32_t)__abs(point)) > MAX_SDLL_VAL))) {
	 shim_printf("Trying to set a WR DQS code which is more than 4*MDLL, Code=%d, Limit=%d \n", __abs(point), (4 * mdllcode_byte));
	 wrdqcal_vref_panic[wrdqcal_chan][wrdqcal_vref] = 1;
    return -1;
  } else {
  		if (((uint32_t)__abs(point)) > (5 * mdllcode_byte)) {
			shim_printf("Trying to set a WR DQ code which is more than 4*MDLL (DQ deskew can hold a code of MDLL), Code=%d, Limit=%d \n", __abs(point), (4 * mdllcode_byte));
			wrdqcal_vref_panic[wrdqcal_chan][wrdqcal_vref] = 1;
    		return -1;
  		}
  }
  ...
  if (point >= 0) {
    ...
    if ( (((uint32_t)point) > (MAX_SDLL_VAL + MAX_DESKEW_PROGRAMMED) ) || (((uint32_t)point) > (5*mdllcode_byte)) ) {
      shim_panic("Trying to set a WR DQ/DQS code which is more than maximum allowed \n");
		wrdqcal_vref_panic[wrdqcal_chan][wrdqcal_vref] = 1;
      return -1;
     }
    else {
       // Compute Min(4*MDLL, 248)
       if ((4*mdllcode_byte) < MAX_SDLL_VAL) {
         min_of_4mdll_maxsdll = 4*mdllcode_byte;
         } 
       else if ((4*mdllcode_byte) >= MAX_SDLL_VAL){
         min_of_4mdll_maxsdll =  MAX_SDLL_VAL;
       }
...
_ROMCODE_SEGMENT_PREFIX void wrdqcal_sequence(uint8_t chan, uint8_t rank)
{
  ...
  if(cfg_params.wrdq_apply_NC_algorithm) {
    	...
    	// determine the threshold to switch to n = 1
    	// MAUI_WAIVE : Making threshold as 10*MDLL to aviod having N=1
    	SDLL_threshold_n = (10 * mdllcode[chan][AMP_CA]);

    	// Search for Passing Zone
    	// Find Passing Zone: Start @ZERO & Coarse-Step Left (+) ... Find "enough" Consecutive Pass-Points
    	for(point = 0; point < SDLL_threshold_n; point += COARSE_STEP_SZ) {
			// FIXME (MAUI_WAIVE): should be 8-in-a-row? or 2-in-a-row?
			if (wrdqcal_solid_pass(both_bytes, point, 1, 2)) {
				// In Phase 2, swtch to fine stepping once we reach point=3*MDLL-8
	  			for(; point < SDLL_threshold_n; point += COARSE_STEP_SZ) {
...
_ROMCODE_SEGMENT_PREFIX void wrdqcal_program_final_values(void)
{
	...
	for (ch = 0; ch < cfg_params.num_channels; ch++) {
		...
		if(cfg_params.chip_id == 0x8000 && cfg_params.chip_rev < CHIP_REVISION_B0) {
			if(cfg_params.freq_bin == 1) {
				freqchngctl0 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL0_FREQ1(ch));
				CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL0_FREQ1(ch), (freqchngctl0  & 0xFFFF) | (0xE << 16) | (cfg_params.wrdq_vref_values[vref_ch] << 24));
			}
			else if(cfg_params.freq_bin == 0) {
				freqchngctl0 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL0_FREQ0(ch));
				CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL0_FREQ0(ch), (freqchngctl0  & 0xFFFF) | (0xE << 16) | (cfg_params.wrdq_vref_values[vref_ch] << 24));
			}
		} else {
			if(cfg_params.freq_bin == 1) {
				freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch));
				CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ1(ch), (freqchngctl1	& 0xFFFF0000) | (0xE) | (cfg_params.wrdq_vref_values[vref_ch] << 8));
			}
			else if(cfg_params.freq_bin == 0) {
				freqchngctl1 = CSR_READ(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch));
				CSR_WRITE(rAMCX_DRAMCFG_FREQCHNGCTL1_FREQ0(ch), (freqchngctl1	& 0xFFFF0000) | (0xE) | (cfg_params.wrdq_vref_values[vref_ch] << 8));
			}
		}
...
_ROMCODE_SEGMENT_PREFIX void amp_compute_opt_vref_cacal(uint32_t ch, uint32_t rnk, uint32_t swloop, uint32_t bitMax)
{
	...
	amp_compute_opt_vref("CA", ca_num_vref, bitMax,
...
_ROMCODE_SEGMENT_PREFIX void amp_compute_opt_vref_rddqcal(uint32_t ch, uint32_t rnk, uint32_t bitMax)
{
	...
	// Run the Calculation for the Optimal VRef and Optimal Center
	amp_compute_opt_vref("RdDQ", rddq_num_vref, bitMax,
