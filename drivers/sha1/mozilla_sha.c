...
#if (!WITH_CORECRYPTO || !WITH_VFP) && !WITH_HW_SHA1
...
void shaUpdate(SHA_CTX *ctx, unsigned char *dataIn, u_int32_t len) {
  u_int32_t i;
  ...
  for (i = 0; i < len; i++) {
    ctx->W[ctx->lenW / 4] <<= 8;
    ctx->W[ctx->lenW / 4] |= (u_int32_t)dataIn[i];
    if ((++ctx->lenW) % 64 == 0) {
      shaHashBlock(ctx);
      ctx->lenW = 0;
    }
...
#endif /* (!WITH_CORECRYPTO || !WITH_VFP) && !WITH_HW_SHA1 */
