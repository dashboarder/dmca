...
#define HFSPLUS_ARCHIVED_FOLDERS  ".Archived_Directories\x0d"
...
/*
 * Files in the ".Archived_Directories" folder have the following prefix
 * followed by a decimal number (no leading zeros) for the file ID.
 *
 * e.g. dir_555
 */
...
struct HFSPlusBSDInfo {
	uint32_t 	ownerID;	/* user-id of owner or hard link chain previous link */
	uint32_t 	groupID;	/* group-id of owner or hard link chain next link */
	uint8_t 	adminFlags;	/* super-user changeable flags */
	uint8_t 	ownerFlags;	/* owner changeable flags */
	uint16_t 	fileMode;	/* file type and permission bits */
	union {
	    uint32_t	iNodeNum;	/* indirect node number (hard links only) */
	    uint32_t	linkCount;	/* links that refer to this indirect node */
	    uint32_t	rawDevice;	/* special file device (FBLK and FCHR only) */
	} special;
} __attribute__((aligned(2), packed));
...
struct HFSCatalogKey {
	...
	uint32_t 	parentID;		/* parent folder ID */
	uint8_t 	nodeName[kHFSMaxFileNameChars + 1]; /* catalog node name */
} __attribute__((aligned(2), packed));
...
enum {
	...
	kHFSHasFolderCountBit	= 0x0004,	/* folder maintains a seperate sub-folder count */
...
struct HFSPlusCatalogFolder {
	...
	uint32_t 		valence;		/* folder's item count */
	uint32_t 		folderID;		/* folder ID */
	uint32_t 		createDate;		/* date and time of creation */
	...
	uint32_t 		accessDate;		/* date and time of last access (MacOS X only) */
	uint32_t 		backupDate;		/* date and time of last backup */
	HFSPlusBSDInfo		bsdInfo;		/* permissions (for MacOS X) */
	FndrDirInfo 		userInfo;		/* Finder information */
	FndrOpaqueInfo	 	finderInfo;		/* additional Finder information */
	uint32_t 		textEncoding;		/* hint for name conversions */
	uint32_t 		folderCount;		/* number of enclosed folders, active when HasFolderCount is set */
} __attribute__((aligned(2), packed));
...
struct HFSPlusCatalogFile {
	...
	uint16_t 		flags;			/* file flags */
	uint32_t 		reserved1;		/* reserved - initialized as zero */
	uint32_t 		fileID;			/* file ID */
	uint32_t 		createDate;		/* date and time of creation */
	uint32_t 		contentModDate;		/* date and time of last content modification */
	uint32_t 		attributeModDate;	/* date and time of last attribute modification */
	...
	FndrOpaqueInfo	 	finderInfo;		/* additional Finder information */
	uint32_t 		textEncoding;		/* hint for name conversions */
	uint32_t 		reserved2;		/* reserved - initialized as zero */
...
struct HFSCatalogThread {
	...
	uint32_t 	parentID;		/* parent ID for this catalog node */
	uint8_t 	nodeName[kHFSMaxFileNameChars + 1]; /* name of this catalog node */
} __attribute__((aligned(2), packed));
...
struct HFSPlusAttrData {
	...
	uint32_t    reserved[2];
	uint32_t    attrSize;     /* size of attribute data in bytes */
	uint8_t     attrData[2];  /* variable length */
} __attribute__((aligned(2), packed));
...
struct HFSPlusAttrKey {
	uint16_t     keyLength;       /* key length (in bytes) */
	uint16_t     pad;	       /* set to zero */
	uint32_t     fileID;          /* file associated with attribute */
...
enum {
	...
	kHFSCatalogKeyMinimumLength	= kHFSCatalogKeyMaximumLength - (kHFSMaxFileNameChars + 1) + sizeof(uint8_t),
...
struct HFSMasterDirectoryBlock {
	...
	uint8_t 		drVN[kHFSMaxVolumeNameChars + 1];  /* volume name */
...
struct BTNodeDescriptor {
	...
	uint16_t 	reserved;		/* reserved - initialized as zero */
} __attribute__((aligned(2), packed));
...
struct BTHeaderRec {
	...
	uint8_t 	keyCompareType;		/* Key string Comparison Type */
