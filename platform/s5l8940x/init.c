...
int platform_early_init(void)
{
	...
	/* initialize the s5l8940x pmgr driver */
	platform_power_init();
#endif
...
{
	if ((platform_get_chip_id() == 0x8940) && (platform_get_chip_revision() < 0x21))
...
void platform_mmu_setup(bool resume)
{
	...
	arm_mmu_map_section_range(SRAM_BASE, SRAM_BASE, ROUNDUP(SRAM_BANK_LEN, MB)/MB, kARMMMUNormal, false, true);
#if APPLICATION_SECUREROM
...
bool platform_get_boot_device(int32_t index, enum boot_device *boot_device, u_int32_t *boot_flag, u_int32_t *boot_arg)
{
	...
	/* S5L8940X supports one boot device then USB-DFU per boot config */
...
#if WITH_EFFACEABLE && PRODUCT_IBOOT && SUB_PLATFORM_S5L8947X
...
#ifndef TARGET_CPU_VOLTAGE_MASK
#define TARGET_CPU_VOLTAGE_MASK 7
#endif

static const u_int32_t cpu_voltage_points[3] = {
#if TARGET_CPU_VOLTAGE_MASK & 1
	CHIPID_CPU_VOLTAGE_LOW,
#endif
#if TARGET_CPU_VOLTAGE_MASK & 2
	CHIPID_CPU_VOLTAGE_MED,
#endif
#if TARGET_CPU_VOLTAGE_MASK & 4
	CHIPID_CPU_VOLTAGE_HIGH,
#endif
};
