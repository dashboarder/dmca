...
static int32_t platform_bootprep_darwin(void);
...
#if SUB_PLATFORM_S8000 || SUB_PLATFORM_S8001
...
#if WITH_HW_KSF_BACKLIGHT
	// Wait 20 ms  for the backlight to ramp up in color while showing black frames.
	// <rdar://problem/24329068> J127 red flash when pressing sleep button with white Upgrade screen
	spin(20000);
#endif
#endif
...
int platform_bootprep(enum boot_target target)
{
	...
	switch (target) {
		...
		case BOOT_DIAGS:
			...
			reconfig_init(target);
			platform_reconfig_sequence_insert(target);
#if !DEBUG_BUILD && SUB_PLATFORM_S8001
			reconfig_lock(BOOT_DIAGS);
#endif
			platform_quiesce_display();
#if WITH_BOOT_STAGE
...
void platform_mmu_setup(bool resume)
{
	...
	// map SecureROM testing region read-write
	arm_mmu_map_rw(SRAM_BASE, HEAP_BASE - SRAM_BASE);
#endif
	arm_mmu_map_rw(HEAP_BASE, HEAP_SIZE);

	// __TEXT is read/execute, __DATA is read/write
	arm_mmu_map_rx((uintptr_t)&_text_start, text_size);
	arm_mmu_map_rw(text_end_aligned, BOOT_TRAMPOLINE_BASE - text_end_aligned);
	// and then boot trampoline is read/execute
	arm_mmu_map_rx(BOOT_TRAMPOLINE_BASE, BOOT_TRAMPOLINE_SIZE);
...
int platform_update_device_tree(void)
{
	...
	if (platform_get_chip_revision() < CHIP_REVISION_B0) {
		if(FindNode(0, "defaults", &node)) {
			propName = "l2-ecc-correctable-panic";
			if(FindProperty(node, &propName, &propData, &propSize)) {
				dprintf(DEBUG_INFO, "defaults: Disabling L2 ECC Single Bit Error Panic\n");
				*(uint32_t *) propData = 0;
			}
		}
		if (dt_find_node(0, "arm-io/apcie", &node)) {
			dt_set_prop(node, "no-nand-sysclk-gating", NULL, 0);
		}
	}
#endif
...
#if	SUB_PLATFORM_S8000
#define	MIN_CHIPREV_FOR_INVALIDATE_FIFO_EMPTY_STATUS	CHIP_REVISION_C0
#elif	SUB_PLATFORM_S8001
#define	MIN_CHIPREV_FOR_INVALIDATE_FIFO_EMPTY_STATUS	CHIP_REVISION_B0
#elif	SUB_PLATFORM_S8003
#define	MIN_CHIPREV_FOR_INVALIDATE_FIFO_EMPTY_STATUS	CHIP_REVISION_A1
#else
#error "Unknown SUB_PLATFORM"
#endif

	if (FindNode(0, "arm-io/nvme-mmu0", &node)) {
		if (platform_get_chip_revision() < MIN_CHIPREV_FOR_INVALIDATE_FIFO_EMPTY_STATUS) {
			// Must use the delay loop method for older chips.
			// DELETEME when Maui B0/Elba A0/Malta A0 are deprecated
			dt_set_prop_32(node, "invalidate-fifo-workaround", 2);
		} else {
			// Can use the MMUFIFOSTS.invalidate_fifo_empty status bit.
			dt_set_prop_32(node, "invalidate-fifo-workaround", 1);
		}
	}
#endif
...
bool platform_get_boot_device(int32_t index, enum boot_device *boot_device, uint32_t *boot_flag, uint32_t *boot_arg)
{
	...
	/* S8000 supports one boot device then USB-DFU per boot config */
...
uint64_t platform_get_memory_size(void)
{
	...
	if ((rPMGR_SCRATCH0 & kPlatformScratchFlagMemoryInfo) != 0) {
		memory_size = (((uint64_t)rPMGR_SCRATCH13) & 0xffff) * 1024 * 1024;
	}
	else {
...
uint8_t platform_get_memory_manufacturer_id(void)
{
	...
	if ((rPMGR_SCRATCH0 & kPlatformScratchFlagMemoryInfo) != 0) {
		return ((rPMGR_SCRATCH13 >> 24) & 0xff);
	}
	else {
		panic("memory not yet inited\n");
	}
...
void platform_set_memory_info_with_revids(uint8_t manuf_id, uint64_t memory_size, uint8_t rev_id, uint8_t rev_id2)
{
	...
	rPMGR_SCRATCH13 = (manuf_id << 24) | ((rev_id2 & 0xF) << 20) | ((rev_id & 0xF) << 16) | (memory_size  & 0xffff);
...
void platform_get_memory_rev_ids(uint8_t *rev_id, uint8_t *rev_id2)
{
	...
	if ((rPMGR_SCRATCH0 & kPlatformScratchFlagMemoryInfo) == 0)
		...
		if (rev_id && rev_id2) {
			uint8_t rev_info = (rPMGR_SCRATCH13 >> 16) & 0xFF;
			*rev_id = rev_info & 0xF;
			*rev_id2 = rev_info >> 4;
		}
	}
...
static int32_t platform_bootprep_darwin(void)
{
...
#if PRODUCT_IBEC && defined(GPIO_S3E_BOOT_FROM_HOST)
	gpio_write(GPIO_S3E_BOOT_FROM_HOST, 1);
#endif
	...
	for (uint32_t i = 0; i < NUM_AMCCS; i++) {
		...
		rMCCLOCKREGION_TZ1BASEADDR(i) = TZ1_BASE >> 12;
		rMCCLOCKREGION_TZ1ENDADDR(i)  = (TZ1_BASE + TZ1_SIZE - 1) >> 12;

		/* lock TZ0 & TZ1 regions */
		rMCCLOCKREGION_TZ0LOCK(i) = (1 << 0);
		if ((rMCCLOCKREGION_TZ0LOCK(i) & 1) == 0) {
			panic("TZ0 failed to lock\n");
		}
		rMCCLOCKREGION_TZ1LOCK(i) = (1 << 0);
		if ((rMCCLOCKREGION_TZ1LOCK(i) & 1) == 0) {
			panic("TZ1 failed to lock\n");
		}
	}

	// Don't allow access to memcache as RAM from the kernel
	dcs_lock_down_mcc();
	...
	// Insert Reconfig sequences
	reconfig_init(BOOT_DARWIN);
	platform_reconfig_sequence_insert(BOOT_DARWIN);
	reconfig_lock(BOOT_DARWIN);
...
{
	// Per Maui tunables spec r0.15 section 6.2
	return 70;
}
...
{
#if SUB_PLATFORM_S8001
	// Per Elba tunables spec r0.39 section 6.2
	return 0;
#else
	// Per Maui tunables spec r0.69 section 6.2
	return 25;
#endif
}
