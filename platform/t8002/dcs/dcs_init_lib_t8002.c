...
// rdar://problem/19137387: this applies to Maui A1 and B0, as well as other SoCs before Cayman
	...
	dcs_reg_write(rAMC_AMCEN, 0x80010001);
	dcs_reg_write(rAMC_AIUADDRBANKHASH(2), 0x000036db);
	dcs_reg_write(rAMC_AIUADDRBANKHASH(1), 0x00005b6d);
	dcs_reg_write(rAMC_AIUADDRBANKHASH(0), 0x00006db6);
	dcs_reg_write(rAMC_ADDRMAPMODE, 0x00000002);
	dcs_reg_write(rAMC_ADDRCFG, 0x00030201);
...
void dcs_init_AMP_Initial_Config(void)
{
	...
	dcs_reg_write_all_chan(rDCS_AMP_MAXWRDQ_SDLL_MULFACTOR, 0x00a00f0f);
	...
	dcs_reg_write_all_chan(rDCS_AMP_IMPCALCONTROL3, 0x00000000);
...
// AMPH configuration
	...
	dcs_reg_write_all_chan(rDCS_AMPH_CB_IMPCTL,     0x0f001c2b);
	dcs_reg_write_all_chan(rDCS_AMPH_CK_WKPUPD,     0x00000000);
...
void dcs_init_Self_Refresh_Exit_a(void)
{
	...
	dcs_reg_write_all_chan(rDCS_MCU_DIV_CLK_SELECT, 0x00000000);
	dcs_reg_write_all_chan(rDCS_SPLLCTRL_RECONFIGSWRESETCONTROL, 0x0000003c);
	dcs_reg_write_all_chan(rDCS_SPLLCTRL_RECONFIGSWRESETTRIGGER, 0x00000001);
	// Wait 20ns
	dcs_spin(1, NULL);
	dcs_reg_poll(rAMC_PIO_ACCESS_CFG, 0x1, 0x1);
...
// MR12 register
		dcs_mrcmd(MR_WRITE, dcs_cfg.num_dcs, dcs_cfg.num_rank, 0xc, 0x4d);
		// MR14 register
		dcs_mrcmd(MR_WRITE, dcs_cfg.num_dcs, dcs_cfg.num_rank, 0xe, 0x4d);
		// MR23 register
...
{
	dcs_reg_write(rAMC_ADDRCFG, dcs_params.addrcfg);
...
void dcs_init_TotalSize_Cfg(uint32_t total_size_Mbytes)
{
	...
	dbgprintf(DCS_DBG_ASSESS_CONFIG, "Memory_size: %llu bytes\n",
					(((uint64_t)total_size_Mbytes) * MEG));
	dcs_reg_write(rAMC_AIUPRTBADDR, dcs_params.aiuprtbaddr);
	dcs_reg_write(rAMC_AIUCHNLDEC, dcs_params.chnldec);
...
{
	dcs_reg_write_all_chan(rDCS_AMPH_SPARE(0), 0x0000040b);
...
void dcs_init_MoreFeatures(void)
{
	...
	dcs_reg_write(rAMC_QBREN, 0x00110000);
	dcs_reg_write_all_chan(rDCS_MCU_QBREN, 0x00000001);
	...
	dcs_reg_write_all_chan(rDCS_AMPH_DQS0_DCD_IOCTL, 0x00010000);
	dcs_reg_write_all_chan(rDCS_AMPH_DQS1_DCD_IOCTL, 0x00090000);	
...
void dcs_init_Fast_Critical_Word_Forwarding(void)
{
	...
	uint32_t aiuprt_rd_cwf = 0x00121200;
	uint32_t qbrparam2 = 0x00004555;

	if(izFPGA) {
		aiuprt_rd_cwf = 0x00000000;
		qbrparam2 = 0x00000555;
	}

	dcs_reg_write(rAMC_AIUPRT_RD_CWF, aiuprt_rd_cwf);
	dcs_reg_write_all_chan(rDCS_MCU_QBRPARAM2, qbrparam2);
...
void dcs_init_Gating_Global_Timers(void)
{
	...
	if(!izFPGA) {
		dcs_reg_write(rAMC_IDLE2BCG_CONTROL, 0x00000001);
		dcs_reg_write_all_chan(rDCS_AON_IDLE2BCG_CONTROL, 0x00000001);
		dcs_reg_write_all_chan(rDCS_MCU_IDLE_EXTEND_CONTROL, 0x00000001);
		dcs_reg_write_all_chan(rDCS_MCU_AMCCLKPWRGATE, 0x050a1100);
		dcs_reg_write(rAMC_PWRMNGTPARAM, 0x001d0140);
		dcs_reg_write(rAMC_PSQWQCTL1, 0x00240100);
	}	
	dcs_reg_write_all_chan(rDCS_AMP_DQCALBURSTLENMODE, 0x00000011);


	// Global Timer Setup
	dcs_reg_poll(rGLBTIMER_FREQCHNG_ACTIVE, 0x1, 0x0);
	...
	dcs_reg_write(rGLBTIMER_RDCALTIMER,           0x2dc6bb);	//
...
void dcs_init_ODTS(void)
{
	...
	// disable full scan calibrations
	if(!izFPGA)
