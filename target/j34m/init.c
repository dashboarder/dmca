...
// DEV Boards
#define	J34M_DEV0_BOARD		(0x0)	//Hoover Only
#define	J34M_DEV1_BOARD		(0x1)	//Hoover Only
#define	J34M_DEV2_BOARD		(0x2)	//Hoover Only
//	...define other DEV Boards here...
#define	J34M_DEV_BOARD_LAST	(0x7)	//Hoover Only

// AP Boards
#define	J34M_AP_BOARD_FIRST	(0x8)	//Hoover Only
//	...define AP Boards here...
#define	J34M_DVT_BOARD		(0xC)	//Hoover Only
#define	J34M_PVT_BOARD		(0xF)	//Hoover Only
...
static u_int32_t j34m_get_board_rev(void)
{
...
static void j34m_reset_hoover(bool reset)
{
...
void target_late_init(void)
{
	...
	if (target_config_ap() && j34m_get_board_rev() < J34M_AP_BOARD_FIRST) {
		platform_not_supported();
	}
...
int target_update_device_tree(void)
{
	...
	if (FindNode(0, "arm-io/mcu0", &node)) {
		propName = "board-rev";
		if (FindProperty(node, &propName, &propData, &propSize))
			((u_int32_t *)propData)[0] = j34m_get_board_rev();
	}
