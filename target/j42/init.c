...
// Board ids
#define J42_AP_BOARD_ID			(0x14)
#define J42_DEV_BOARD_ID		(0x15)
#define J42M_AP_BOARD_ID		(0x16)
#define J42M_DEV_BOARD_ID		(0x17)
#define J42D_AP_BOARD_ID		(0x34)
#define J42D_DEV_BOARD_ID		(0x35)

// Board revs
#define J42_DEV1_BOARD_REV		(0x7)
#define J42_PROTO1_BOARD_REV		(0x6)
#define J42_EVT1_BOARD_REV		(0x4)
...
#define kDPDeviceStartTimeout (10 * 1000 * 1000)
...
static uint32_t j42_get_board_rev(void)
{
...
static void j42_reset_hoover(bool reset)
{
...
{
#if 0
	if (target_config_dev() && j42_get_board_rev() > J42_DEV_BOARD_LAST) {
		platform_not_supported();
	}
	if (target_config_ap() && j42_get_board_rev() < J42_AP_BOARD_FIRST) {
		platform_not_supported();
	}
#endif
...
bool target_should_recover(void)
{
	...
	// <rdar://problem/18089191> J42: USB VBUS detection should be changed from PMU to PMU GPIO	
	return power_get_gpio(POWER_GPIO_VBUS_PROT_2V5) && !env_get_bool("auto-boot-usb", true);
}
...
void target_setup_default_environment(void)
{
	...
	env_set("display-timing", "720p", 0); // XXX DBJ do we need updated timing table in platform/t7000/include/platform/soc/display_timings.h for j42?
	...
	env_set("adfe-tunables", "default", 0);
}
...
int target_update_device_tree(void)
{
	...
	if (FindNode(0, "arm-io/mcu0", &node)) {
		propName = "board-rev";
		if (FindProperty(node, &propName, &propData, &propSize))
			((u_int32_t *)propData)[0] = j42_get_board_rev();
	}
